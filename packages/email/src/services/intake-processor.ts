...
import { PLAN_CLASSIFICATIONS } from '../types/classification'
  ...
  async processNewEmail(workspaceId: string, messageId: string): Promise<void> {
    try {
      ...
      if (mailMessage.ingestionMode === 'personal') {
        ...
        if (!hasOptedIn) {
          this.logToCloudWatch(
            'intake-processor',
            'Personal email skipped - user has not opted in',
            {
              workspaceId,
              messageId,
        ...
        this.logToCloudWatch(
          'intake-processor',
          'Personal email processing with privacy controls',
          {
            workspaceId,
            messageId,
            mailbox: mailMessage.mailbox,
      ...
      if (classification.confidence < 0.6) {
        this.logToCloudWatch(
          'intake-processor',
          'Low confidence classification, marking for review',
          {
            workspaceId,
            messageId,
            confidence: classification.confidence,
      ...
      if (agentResponse.artifacts && agentResponse.artifacts.length > 0) {
        for (const artifact of agentResponse.artifacts) {
          try {
            await artifactService.createArtifact(runId, artifact.type, artifact.content, {
              sourceMessageId: messageId,
            })
